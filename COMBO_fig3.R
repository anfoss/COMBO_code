...
## Fig3A
dd = read.csv('data/infl_marker.csv')
dd = subset(dd,dd$TB.Classification !='Latent TB Control')
dd = subset(dd,dd$TB.Classification !='')

my_comparisons <- list( c("Unlikely TB", "Healthy Control"), 
                        c("Confirmed TB",  "Healthy Control"), c("Unconfirmed TB", "Healthy Control"))

p = ggboxplot(dd, "TB.Classification", "value",add = "jitter",color = "TB.Classification", palette = "lancet", add.params = list(size = .1, alpha = 0.5, shape=20))+
  facet_wrap(~Genes, ncol = 4, nrow = 1, scale='free')+ 
  stat_compare_means(comparisons = my_comparisons, aes(label = ..p.signif..))+
  ylab('Protein Intensity')+theme_Publication()+rmback+fix_plot+ theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))+
  theme(legend.position = 'none')+theme(axis.title.x=element_blank())+theme(axis.text.x  = element_text(size=7))+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
ggsave(filename = 'Fig3A.pdf', plot = p, height = 4, width = 6)
...
ggsave(filename = 'Fig3B.pdf', plot = p, height = 1.5, width = 1.5)
...
ggsave(plot=p,filename =  'Fig3B.pdf', width = 1.25, height = 1.25)



### Fig 3C
mycol = c('Confirmed TB'='#d9cc26','Unlikely TB'='#d86d27', 'Unconfirmed TB'='#2792D8')
dd = read.csv('data/z_score_dep_top20.csv')
dd = subset(dd,dd$Gene %in% c('IGHV3-30'))
p = ggplot(dd, aes(x=val_norm,fill=TB.Classification, col=TB.Classification))+
  geom_density(alpha=0.5)+
  theme_Publication()+rmback+fix_plot + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))+
  xlab('Z-score')+scale_fill_manual(values=mycol)+scale_colour_manual(values=mycol)+
  theme(legend.position = 'bottom', legend.direction = 'horizontal')+scale_y_continuous(expand=c(0,0))+
  scale_x_continuous(expand=c(0,0))
p
ggsave(plot=p,filename =  'Fig3D.pdf', width = 2.5, height = 3.5)
p
